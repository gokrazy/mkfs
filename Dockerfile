...
RUN apt-get update && apt-get install -y crossbuild-essential-armhf crossbuild-essential-arm64 build-essential curl
...
RUN cd /src && ./configure CFLAGS='-O2 -static' LDFLAGS=-static CC=arm-linux-gnueabihf-gcc --host=arm-linux-gnueabihf && make -j$(nproc) && cp ./misc/mke2fs /bins/mke2fs.arm && make distclean
